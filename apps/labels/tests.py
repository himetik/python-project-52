...
class LabelCreateViewTest(TestCase):
    ...
    def test_create_label_success(self):
        response = self.client.post(reverse('labels_create'), {'name': 'Another Label'}, follow=True)
        self.assertIn('The label has been successfully created', response.content.decode())
    ...
    def test_create_view_redirects_if_not_logged_in(self):
        ...
        self.assertTrue(
            response.url == reverse("login") or response.url.startswith(f"{reverse('login')}?next="),
...
class LabelUpdateViewTest(TestCase):
    ...
    def test_update_label_view_status_code(self):
        response = self.client.get(reverse('labels_update', kwargs={'pk': self.label.pk}))
        self.assertEqual(response.status_code, 200)

    def test_update_label_success(self):
        response = self.client.post(reverse('labels_update', kwargs={'pk': self.label.pk}), {'name': 'Updated Label'}, follow=True)
        self.label.refresh_from_db()
        self.assertEqual(self.label.name, 'Updated Label')
        self.assertIn('The label has been successfully changed', response.content.decode())
...
class LabelDeleteViewTest(TestCase):
    ...
    def test_delete_label_view_status_code(self):
        response = self.client.get(reverse('labels_delete', kwargs={'pk': self.label.pk}))
        self.assertEqual(response.status_code, 200)

    def test_delete_label_success(self):
        response = self.client.post(reverse('labels_delete', kwargs={'pk': self.label.pk}), follow=True)
        self.assertFalse(Label.objects.filter(pk=self.label.pk).exists())
        self.assertIn('The label has been successfully deleted', response.content.decode())
    ...
    def test_delete_label_with_tasks(self):
        ...
        self.assertIn('Cannot delete the label because it is currently in use', response.content.decode())
